"""
Main Entry Point for Email Outreach Agent.
Author: Danish (Dan-445)
"""
import streamlit as st
import os

from config import OPENAI_API_KEY, EXA_API_KEY, APP_TITLE, setup_logging
...
def main() -> None:
    ...
    exa_key = st.sidebar.text_input("Exa API Key", type="password", value=EXA_API_KEY)
    
    # Update env vars for the agents to use if Agno relies on them internally 
    # (though our create_agent functions pass them if we modified them to, 
    # but currently they use default env loading or we need to ensure they are set).
    if openai_key:
    ...
    st.info(
        ...
        "This app uses AI Agents to find target companies, identify contacts, research genuine insights (website + Reddit), "
