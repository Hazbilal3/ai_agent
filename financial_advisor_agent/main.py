"""
Main entry point for the AI Financial Advisor Agent.
...
import pandas as pd
# dotenv loaded in config

from agents import FinanceAdvisorSystem
from utils import validate_csv_format, parse_csv_transactions
from ui import (
    display_budget_analysis,
    display_savings_strategy,
    display_debt_reduction,
    display_csv_preview,
    load_custom_css
)
from config import (
...
def main():
    ...
    if not BYTEZ_API_KEY:
        st.error("🔑 BYTEZ_API_KEY not found in environment variables. Please add it to your .env file.")
        st.stop()
    ...
    with st.sidebar:
        ...
        st.caption("This application uses Bytez API and Open Source Models to provide personalized financial advice.")
...
2024-01-03,Transportation,45.00"""
    ...
    st.caption("Powered by Bytez API")
    st.markdown("This tool analyzes your financial data and provides tailored recommendations for budgeting, savings, and debt management using specialized AI agents.")
    st.divider()
    ...
    with input_tab:
            ...
            if expense_option == "📤 Upload CSV Transactions":
                ...
                if manual_expenses and any(manual_expenses.values()):
                    ...
                    if not manual_df_disp.empty:
                        col1_man, col2_man = st.columns([2, 1])
                        with col1_man:
                            st.dataframe(
                                manual_df_disp,
                                column_config={
        ...
        with st.container():
            ...
            if num_debts > 0:
                ...
                cols = st.columns(min(num_debts, 3)) if num_debts < 4 else st.columns(3)
        ...
        if analyze_button:
            ...
            elif use_manual_expenses and (not manual_expenses or not any(manual_expenses.values())):
    ...
    with about_tab:
        ...
        This application uses Bytez API to provide comprehensive financial analysis and advice through multiple specialized AI agents:
        ...
        - Secure API communication with Bytez services
