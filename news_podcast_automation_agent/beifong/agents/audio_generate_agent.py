...
logger = get_logger("audio_generate_agent")
...
def create_silence_audio(silence_duration: float, sampling_rate: int) -> np.ndarray:
    if sampling_rate <= 0:
        logger.warning(f"Invalid sampling rate ({sampling_rate}) for silence generation")
...
def process_audio_file(temp_path: str) -> Optional[Tuple[np.ndarray, int]]:
    ...
    try:
        ...
        logger.warning(f"Failed to process audio with soundfile: {e}")
        try:
            from pydub import AudioSegment
            ...
            logger.error(f"All audio processing methods failed: {e}")
    return None
...
def text_to_speech_openai(
    ...
    if not voice:
        ...
        logger.info(f"No voice mapping for speaker {speaker_id}, using {voice}")
    try:
        logger.info(f"Generating TTS for speaker {speaker_id} using voice '{voice}'")
        ...
        if not audio_data:
            ...
            return None
        logger.info(f"Received {len(audio_data)} bytes from OpenAI TTS")
...
def create_podcast(
    ...
    if tts_engine.lower() != "openai":
        logger.error(f"Only OpenAI TTS engine is available in this standalone version. Requested: {tts_engine}")
        return None
    try:
    ...
    if model == "tts-1" and language_code == "en":
        ...
        logger.info(f"Using high-definition TTS model for English: {model_to_use}")
    generated_segments = []
    ...
    logger.info(f"Processing {len(entries)} script entries")
    for i, entry in enumerate(entries):
        ...
        logger.info(f"Processing entry {i + 1}/{len(entries)}: Speaker {speaker_id}")
        ...
        if result:
            ...
            if sampling_rate_detected is None:
                ...
                logger.info(f"Using sample rate: {sampling_rate_detected} Hz")
            elif sampling_rate_detected != segment_rate:
                logger.warning(f"Sample rate mismatch: {sampling_rate_detected} vs {segment_rate}")
                try:
                    segment_audio = resample_audio(segment_audio, segment_rate, sampling_rate_detected)
                    logger.info(f"Resampled to {sampling_rate_detected} Hz")
                except Exception as e:
                    sampling_rate_detected = segment_rate
                    logger.error(f"Resampling failed: {e}")
            generated_segments.append(segment_audio)
        else:
            logger.error(f"Failed to generate audio for entry {i + 1}")
    if not generated_segments:
        logger.error("No audio segments were generated")
        return None
    if sampling_rate_detected is None:
        logger.error("Could not determine sample rate")
        return None
    logger.info(f"Combining {len(generated_segments)} audio segments")
    full_audio = combine_audio_segments(generated_segments, silence_duration, sampling_rate_detected)
    if full_audio.size == 0:
        logger.error("Combined audio is empty")
        return None

    try:
        if os.path.exists(INTRO_MUSIC_FILE):
            intro_music, intro_sr = sf.read(INTRO_MUSIC_FILE)
            logger.info(f"Loaded intro music: {len(intro_music) / intro_sr:.1f} seconds")
        ...
        if os.path.exists(OUTRO_MUSIC_FILE):
            ...
            logger.info(f"Loaded outro music: {len(outro_music) / outro_sr:.1f} seconds")
        ...
        logger.warning(f"Could not add intro/outro music: {e}")
        logger.info("Continuing without background music")

    logger.info(f"Writing audio to {output_path}")
    try:
    ...
    if os.path.exists(output_path):
        ...
        logger.info(f"Audio file created: {output_path} ({file_size / 1024:.1f} KB)")
        return output_path
    else:
    ...
    try:
        if isinstance(script_data, dict) and "sections" in script_data:
            ...
            if tts_engine == "openai" and not load_api_key("OPENAI_API_KEY"):
                ...
                logger.error(error_msg)
                return error_msg
            logger.info(f"Generating podcast audio using {tts_engine} TTS engine in {language_name} language")
            ...
            logger.info(f"Successfully generated podcast audio: {full_audio_path}")
