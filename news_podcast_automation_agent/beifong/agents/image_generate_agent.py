...
load_dotenv()
logger = get_logger("image_generate_agent")
...
def download_images(image_urls):
    ...
    try:
        ...
        logger.error(f"Error downloading images (network): {e}")
...
def image_generation_agent_run(agent: Agent, query: str) -> str:
    ...
    logger.info(f"Image Generation Agent input: {query}")
    ...
    try:
        ...
        script_dump = json.dumps(session_state.get('generated_script', {}), default=str)
        
        image_agent.run(f"query: {query},\n podcast script: {script_dump}", session_id=agent.session_id)
        ...
        logger.error(f"Error in Image Generation Agent: {e}")
